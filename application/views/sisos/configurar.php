...
<style>
.cg-section{background:#1a1d2e;border:1px solid rgba(255,255,255,0.07);border-radius:14px;overflow:hidden;margin:0 0 14px;}
.cg-section-head{display:flex;align-items:center;gap:8px;padding:11px 18px;border-bottom:1px solid rgba(255,255,255,0.06);background:#252a3a;}
.cg-section-head i{font-size:15px;color:#fbbf24;}
.cg-section-head span{font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.8px;}
.cg-section-body{padding:18px;}
.cg-field{margin-bottom:16px;}
.cg-field:last-child{margin-bottom:0;}
.cg-label{font-size:12px;font-weight:700;color:#c9cad6;display:block;margin-bottom:6px;}
.cg-hint{font-size:11px;color:#6b7280;margin-top:4px;line-height:1.5;}
.cg-input,.cg-select{background:#13151f;border:1px solid #444860;color:#e8eaf0;border-radius:8px;padding:9px 13px;font-size:13px;transition:border-color .15s;-webkit-appearance:none;}
.cg-input{width:100%;display:block;box-sizing:border-box;}
.cg-input:focus,.cg-select:focus{border-color:#fbbf24;outline:none;}
.cg-select{height:38px;min-width:160px;}
.cg-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px;}
@media(max-width:640px){.cg-grid{grid-template-columns:1fr;}}
.cg-logo-row{display:flex;align-items:center;gap:12px;padding:10px 14px;background:#252a3a;border-radius:10px;border:1px solid rgba(255,255,255,0.06);margin-bottom:10px;}
.cg-logo-row img{max-height:52px;max-width:180px;object-fit:contain;}
.cg-upload-row{display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-bottom:6px;}
.cg-btn-upload{display:inline-flex;align-items:center;gap:5px;padding:7px 14px;background:rgba(99,102,241,0.15);color:#a5b4fc;border:1px solid rgba(99,102,241,0.3);border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;transition:all .15s;border-top:none;border-left:none;border-right:none;}
.cg-btn-upload:hover{background:rgba(99,102,241,0.3);}
.cg-msg{font-size:12px;font-weight:600;}
.cg-size-row{display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-top:6px;}
.cg-size-row input[type=number]{width:80px;background:#13151f;border:1px solid #444860;color:#e8eaf0;border-radius:8px;padding:7px 10px;font-size:13px;text-align:center;}
...
<div style="padding:18px 4px 4px;">
    ...
    <div class="cg-section">
        <div class="cg-section-head"><i class='bx bx-paint'></i><span>Identidade Visual</span></div>
        <div class="cg-section-body">

            <div class="cg-field">
                <label class="cg-label">Nome do Sistema</label>
                <input type="text" name="app_name" class="cg-input" required
                       value="<?= htmlspecialchars($configuration['app_name']) ?>"
                       placeholder="Ex: SISOS Assistência Técnica" style="max-width:400px;" />
                <div class="cg-hint">Nome exibido no título do sistema e e-mails.</div>
            </div>

            <!-- Logo -->
            <div class="cg-field">
                <label class="cg-label">Logo do Sistema</label>
                <?php if (!empty($configuration['app_logo'])): ?>
                <div class="cg-logo-row">
                    <img src="<?= $configuration['app_logo'] ?>" alt="Logo atual" id="previewLogoSize"
                         style="max-height:<?= $configuration['app_logo_height'] ?? 50 ?>px;">
                    <span style="font-size:11px;color:#6b7280;">Logo atual</span>
                </div>
                <?php endif; ?>
                <div class="cg-upload-row">
                    <input type="file" id="inputLogo" accept=".png,.jpg,.jpeg,.svg,.webp"
                           style="background:#13151f;border:1px solid #444860;color:#e8eaf0;border-radius:8px;padding:6px 10px;font-size:12px;" />
                    <button type="button" class="cg-btn-upload"
                            onclick="uploadArquivo('inputLogo','<?= site_url('notafiscal/uploadLogo') ?>','msgLogo')">
                        <i class='bx bx-upload'></i> Enviar Logo
                    </button>
                    <span id="msgLogo" class="cg-msg"></span>
                </div>
                <div class="cg-hint">PNG, JPG ou SVG — Máx 2MB.</div>
                <?php if (!empty($configuration['app_logo'])): ?>
                <div class="cg-size-row">
                    ...
                    <span>px</span>
                    <span class="cg-hint" style="margin-top:0;">Prévia acima atualiza em tempo real.</span>
                </div>
                <?php endif; ?>
            </div>
            ...
            <!-- Favicon -->
            <div class="cg-field">
                <label class="cg-label">Favicon / Ícone do Sistema</label>
                <?php if (!empty($configuration['app_favicon'])): ?>
                <div class="cg-logo-row">
                    <img src="<?= $configuration['app_favicon'] ?>" alt="Ícone" style="height:40px;width:40px;object-fit:contain;" />
                    <span style="font-size:11px;color:#6b7280;">Ícone atual — aparece na aba do navegador</span>
                </div>
                <?php endif; ?>
                <div class="cg-upload-row">
                    <input type="file" id="inputFavicon" accept=".png,.ico,.jpg,.jpeg,.svg,.webp"
                           style="background:#13151f;border:1px solid #444860;color:#e8eaf0;border-radius:8px;padding:6px 10px;font-size:12px;" />
                    <button type="button" class="cg-btn-upload"
                            onclick="uploadArquivo('inputFavicon','<?= site_url('notafiscal/uploadFavicon') ?>','msgFavicon')">
                        <i class='bx bx-upload'></i> Enviar Ícone
                    </button>
                    <span id="msgFavicon" class="cg-msg"></span>
                </div>
                <div class="cg-hint">PNG ou ICO — quadrado — Máx 2MB.</div>
            </div>
    ...
    <!-- ── Aparência ── -->
    <div class="cg-section">
        <div class="cg-section-head"><i class='bx bx-palette'></i><span>Aparência</span></div>
        <div class="cg-section-body">
            <div class="cg-grid">
                <div class="cg-field">
                    <label class="cg-label">Tema do Sistema</label>
                    <select name="app_theme" class="cg-select">
                        <option value="default"    <?= $configuration['app_theme']=='default'    ?'selected':'' ?>>🌑 Escuro</option>
                        <option value="white"      <?= $configuration['app_theme']=='white'      ?'selected':'' ?>>☀️ Claro</option>
                    ...
                    <div class="cg-hint">Tema padrão para todos os usuários.</div>
                </div>
                <div class="cg-field">
                    <label class="cg-label">Registros por Página</label>
                    <select name="per_page" class="cg-select">
                        <option value="10"  <?= $configuration['per_page']=='10'  ?'selected':'' ?>>10 por página</option>
                        <option value="20"  <?= $configuration['per_page']=='20'  ?'selected':'' ?>>20 por página</option>
                        <option value="50"  <?= $configuration['per_page']=='50'  ?'selected':'' ?>>50 por página</option>
                        <option value="100" <?= $configuration['per_page']=='100' ?'selected':'' ?>>100 por página</option>
                    </select>
                    <div class="cg-hint">Padrão de registros em listagens.</div>
                </div>
            </div>
            <div class="cg-field" style="margin-top:4px;">
                <label class="cg-label">Visualização em DataTables</label>
                <select name="control_datatable" class="cg-select">
                    <option value="1" <?= $configuration['control_datatable']=='1' ?'selected':'' ?>>✅ Ativado</option>
                    <option value="0" <?= $configuration['control_datatable']=='0' ?'selected':'' ?>>⭕ Desativado</option>
                </select>
                <div class="cg-hint">Ativar tabelas dinâmicas com busca e ordenação nas listagens.</div>
            </div>
        </div>
    </div>
    ...
    <div style="display:flex;justify-content:flex-end;padding:4px 0 8px;">
        <button type="submit" class="cg-btn-save">
