        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($cliente) {
                ...
                if (password_verify($password, $cliente->senha)) {
                    ...
                    echo json_encode(['result' => false, 'message' => 'Os dados de acesso estão incorretos.', 'SISOS_TOKEN' => $this->security->get_csrf_hash()]);
                }
            } else {
                echo json_encode(['result' => false, 'message' => 'Usuário não encontrado, verifique se suas credenciais estão corretas.', 'SISOS_TOKEN' => $this->security->get_csrf_hash()]);
            }
        }
    }
