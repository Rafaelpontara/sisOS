...
class Login extends CI_Controller
{
    ...
    {
        // Carrega configurações do sistema para exibir logo/favicon na tela de login
        $this->load->database();
        ...
        $this->load->view('sisos/login', ['configuration' => $configuration]);
    }
    ...
    public function verificarLogin()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->model('Sisos_model');
            $user = $this->Sisos_model->check_credentials($email);
            ...
            if ($user) {
                ...
                if (password_verify($password, $user->senha)) {
                    ...
                    $json = ['result' => false, 'message' => 'Os dados de acesso estão incorretos.', 'SISOS_TOKEN' => $this->security->get_csrf_hash()];
                    echo json_encode($json);
                }
            } else {
                $json = ['result' => false, 'message' => 'Usuário não encontrado, verifique se suas credenciais estão corretass.', 'SISOS_TOKEN' => $this->security->get_csrf_hash()];
                echo json_encode($json);
            }
        }
        exit();
