...
class ApiController extends REST_Controller
{
    ...
    public function index_get()
    {
        ...
        $result = [
            'countOs' => $this->sisos_model->count('os'),
            'clientes' => $this->sisos_model->count('clientes'),
            'produtos' => $this->sisos_model->count('produtos'),
            'servicos' => $this->sisos_model->count('servicos'),
            'garantias' => $this->sisos_model->count('garantias'),
            'vendas' => $this->sisos_model->count('vendas'),
        ];

        if ($this->permission->checkPermission($this->logged_user()->level, 'vOs')) {
            $result['osAbertas'] = $this->sisos_model->getOsAbertas();
            $result['osAndamento'] = $this->sisos_model->getOsAndamento();
            $result['estoqueBaixo'] = $this->sisos_model->getProdutosMinimo();
        }
    ...
    public function calendario_get()
    {
        ...
        $allOs = $this->sisos_model->calendario($start, $end, $status);
    ...
    public function emitente_get()
    {
        ...
        $result = [
            ...
            'emitente' => $this->sisos_model->getEmitente() ?: false,
        ];

        $this->response([
            'status' => true,
            'message' => 'Dados do SISOS',
            'result' => $result,
        ], REST_Controller::HTTP_OK);
    }
