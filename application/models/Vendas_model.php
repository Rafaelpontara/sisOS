...
class Vendas_model extends CI_Model
{
    ...
    public function getProdutosResumido($id = null)
    {
        $this->db->select('itens_de_vendas.quantidade, produtos.descricao, produtos.codDeBarra');
        $this->db->from('itens_de_vendas');
        $this->db->join('produtos', 'produtos.idProdutos = itens_de_vendas.produtos_id');
        $this->db->where('vendas_id', $id);
        $this->db->limit(3);
    ...
    {
        $this->db->where('codDeBarra', $code);
        $this->db->limit(1);
        $row = $this->db->get('produtos')->row_array();
        if ($row) {
            return [['label' => $row['descricao'] . ' | Preço: R$ ' . $row['precoVenda'] . ' | Estoque: ' . $row['estoque'],
                     'estoque' => $row['estoque'], 'id' => $row['idProdutos'], 'preco' => $row['precoVenda'],
