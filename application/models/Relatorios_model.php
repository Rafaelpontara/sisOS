...
class Relatorios_model extends CI_Model
{
    ...
    public function osCustom($dataInicial = null, $dataFinal = null, $cliente = null, $responsavel = null, $status = null, $array = false, $descricao = null, $referencia = null)
    {
        $whereData = '';
        $whereCliente = '';
        $whereResponsavel = '';
        ...
        if ($descricao != null) {
            $whereDescricao = 'AND (os.descricao LIKE ' . $this->db->escape('%' . $descricao . '%') . ' OR os.defeito LIKE ' . $this->db->escape('%' . $descricao . '%') . ')';
        }
        if ($referencia != null) {
            $whereReferencia = 'AND (os.equipamento LIKE ' . $this->db->escape('%' . $referencia . '%') . ' OR os.numeroSerie LIKE ' . $this->db->escape('%' . $referencia . '%') . ')';
        }
        $query = 'CREATE TEMPORARY TABLE IF NOT EXISTS total_produtos SELECT SUM(subTotal) as total_produto, os_id FROM produtos_os GROUP BY os_id; ';
        $this->db->query($query);
        ...
        $query = "SELECT os.*,clientes.nomeCliente, total_servicos.total_servico, total_produtos.total_produto FROM os
                   ...
                   WHERE idOs != 0 $whereData $whereCliente $whereResponsavel $whereStatus $whereDescricao $whereReferencia
    ...
    public function vendasCustom($dataInicial = null, $dataFinal = null, $cliente = null, $responsavel = null, $array = false, $descricao = null)
    {
        $whereData = '';
        $whereCliente = '';
        $whereResponsavel = '';
        ...
        if ($descricao != null) {
            $whereDescricao = 'AND (vendas.observacoes LIKE ' . $this->db->escape('%' . $descricao . '%') . ' OR vendas.observacoes_cliente LIKE ' . $this->db->escape('%' . $descricao . '%') . ')';
        }
        ...
        WHERE idVendas != 0 $whereData $whereCliente $whereResponsavel $whereDescricao ORDER BY vendas.idVendas";
